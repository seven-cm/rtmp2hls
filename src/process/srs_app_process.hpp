...
//      if ((ret = process->initialize(binary, argv)) != 0) { return ret; }
//      if ((ret = process->start()) != 0) { return ret; }
//      if ((ret = process->cycle()) != 0) { return ret; }
//      process->fast_stop();
//      process->stop();
...
{
public:
	static srs_error_t srs_kill_forced(int& pid);
};
